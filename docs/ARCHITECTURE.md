...
The theming system (`Theme/ThemeManager.swift`, `Theme/Theme.swift`) exposes static palette tokens (`Theme.Palette.*`) as **dynamic colors**. `ThemeManager` always holds *both* a light and a dark palette side-by-side (`dynLight` / `dynDark`), and each token is an `NSColor` dynamic provider that resolves against the current *drawing* appearance. Because AppKit re-resolves dynamic colors whenever the effective appearance changes — the system toggling, or SwiftUI's `preferredColorScheme` — a light↔dark switch recolors the entire window instantly and in place, with no view needing to observe anything. Crucially there is **no `.id()` on the pane tree**: nothing rebuilds, so scroll positions and other transient state survive every theme change (an earlier design keyed `.id` on a revision counter and reset scroll on each tweak).

Non-appearance edits — accent, high-contrast, preset, density, glass transparency — rebuild the two held palettes and bump a `revision` counter. The section containers and `GlassPane` observe `ThemeManager`, so they re-render **in place** (again, no identity change → no scroll reset) and re-read the dynamic colors. Applying an unchanged theme is a full no-op, so window re-activation is free.

Mode (light / dark / system) is the single source of truth for the light/dark base; a custom preset only contributes its accent (plus surface tweaks when its saved base matches), so the Mode switch keeps working while a preset is active. Supported controls: light / dark / system, accent color, high-contrast mode, density, glass transparency (fixed presets — Solid / Subtle / Medium / Heavy — rather than a continuous slider), and custom presets. Text size follows the macOS system setting (re-resolved when the app becomes active) rather than an in-app slider. WCAG contrast helpers guard against low-contrast palettes: `Color.readableText(on:minRatio:)` darkens/lightens accent-derived text just enough to clear AA for any accent, and a debug-only audit logs any remaining low-contrast token pair in *both* appearances.
